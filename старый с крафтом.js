...
(function() {
    ...
    function findEnemyAngle(angle){
        ...
        enemyList.forEach((value, key,)=>{
            ...
            if(dist < 240 && id!==myPlayerId && !checkAlly(id)){
            ...
            if (dist < 500 && dist > 0){
                lastplayers.unshift(dist) && lastplayers.length>5 && lastplayers.pop();
            }
        });
        if(isEnemy){
        ...
        construct:function(target,args){
            ...
            const messageHandler = (e) => {
                if ("string" === typeof e.data){
                    ...
                    switch (d[0]) {
                        case 16:
                            if (circleTimeHp_count===1){
                                circleTimeHp_count=0;
                            }else if (circleTimeHp_count===2){
                                circleTimeHp_count=0;
                                circleTimeHp=Date.now();
                            }else {
                                circleTimeHp_count=1;
                                circleTimeHp=Date.now();
                            }
                            circleTime=Date.now();
        ...
        apply: function(target, _this, _arguments) {
            if(typeof myPlayer==='object' && _arguments[0].tagName=='CANVAS' && arguments.callee.caller.caller.name=='a' ){
                ...
                if (range_pl) {
                    ...
                    _this.beginPath();
                    _this.lineWidth = 5;
                    if (myPlayer.r.x > myPlayer.x) {
                                delete_x=-40
                            } else if (myPlayer.r.x < myPlayer.x) {
                                delete_x=40
                            } else {
                                delete_x=0
                            }
                            if (myPlayer.r.y > myPlayer.y) {
                                delete_y=-40
                            } else if (myPlayer.r.y < myPlayer.y) {
                                delete_y=40
                            } else {
                                delete_y=0
                            }
                    Array.isArray(enemyList) && enemyList.forEach((enemy)=>{
                        ...
                        if(!checkAlly(id) && id!==myPlayer && enemy.type===0){
                            ...
                            _this.moveTo(window.innerWidth / 2 - delete_x, window.innerHeight / 2 - delete_y);
                            _this.lineTo((window.innerWidth / 2) + enemy.x - myPlayer.x - delete_x, (window.innerHeight / 2) + enemy.y - myPlayer.y - delete_y);
                            _this.stroke();
                            _this.stroke();
                            _this.beginPath();
                            _this.arc((window.innerWidth / 2) + enemy.x - myPlayer.x - delete_x, (window.innerHeight / 2) + enemy.y - myPlayer.y - delete_y, 25, 0, 2 * Math.PI, false);
                            _this.stroke();
                        }
                    });
                    _this.strokeStyle = 'green';
                    _this.arc((window.innerWidth / 2) - delete_x, (window.innerHeight / 2) - delete_y, 25, 0, 2 * Math.PI, false);
                    _this.stroke();
                }
                     }catch{}
            }
    ...
    newfunc['canvas.fillRect'] = CanvasRenderingContext2D.prototype.fillRect= new Proxy(CanvasRenderingContext2D.prototype.fillRect, {
        apply: function(target, _this, _arguments) {
            if(arguments.callee.caller && _this.fillStyle==="#669bb1"){
                ...
                _this.font = "31px Arial";
                _this.fillText(`${circleTime? (5-(Date.now()-circleTime)/1000).toFixed(1):'5'}`,_arguments[0]+180,_arguments[1]+20);
                _this.fillText(`${circleTimeHp? (10-(Date.now()-circleTimeHp)/1000).toFixed(1):'10'}`,_arguments[0]-299,_arguments[1]+20);
                if(options.showLastPlayerUI){
                    lastplayers.forEach((p,i)=>{
                        _this.fillText(p,_arguments[0]+180,_arguments[1]+400+i*30);
                    });
                }
                _this.fillStyle="#669bb1"
            }else if(arguments.callee.caller && _this.fillStyle==="#69a148"){
                _this.fillStyle = aimbot? "green" : "red";
                _this.font = "47px Arial";
                _this.fillText(`${events.blizzard.status ?'❄️':' '}`,_arguments[0]-200,_arguments[1]+25);
                _this.fillText(`${events.sandStorm.status ?'🌪️':' '}`,_arguments[0]-200,_arguments[1]-23);
